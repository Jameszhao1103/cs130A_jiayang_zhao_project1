...
int main(int argc, char *argv[]) {
    ...
    int actual_word = total_word;
    for (int i = 0; i < wordlist.size();i++){
        int duplicate = 0;
        duplicate = dict->insertword(numlist[i], wordlist[i]);
        dict->changebucket(numlist[i], duplicate);
        bucket_tracker[numlist[i]] -= duplicate;
        actual_word -= duplicate;
    }
    ...
    //after we clean the duplicates, we output a result page.
    for (int i = 0; i < bucket_size; i++){
        if(bucket_tracker[i]>max_bucket){
            max_bucket = bucket_tracker[i];
            max_bucket_id = i;
        }
    ...
    cout << "Number of words in table: " << actual_word << endl;
